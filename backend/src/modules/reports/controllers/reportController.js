...
class ReportController {
  ...
  async downloadProjectReport(req, res, next) {
    try {
      ...
      const pdfBuffer = await reportService.generateProjectReport(project.toJSON());
  ...
  async downloadPettyCashReport(req, res, next) {
    try {
      const { id } = req.params;
      const { PettyCash, PettyCashEntry, Employee } = require('../../../database/models');
      ...
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename="caja-chica-${pettyCash.id}.pdf"`);
      res.setHeader('Content-Length', pdfBuffer.length);
